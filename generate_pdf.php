...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['EmployeeID'])) {
    ...
    $pdf->SetFont('Arial', '', 8); // Set font size for content
    ...
    $pdf->Cell(0, 10, 'CIVIL STATUS: ' . $employeeData['CivilStatus'], 0, 1); // Added
    $pdf->Cell(0, 10, 'COMPLETE ADDRESS: ' . $employeeData['Address'], 0, 1);
    $pdf->Cell(0, 10, 'HEIGHT: ' . $employeeData['Height'] . ' ft', 0, 1); // Added
    $pdf->Cell(0, 10, 'WEIGHT: ' . $employeeData['Weight'] . ' kg', 0, 1); // Added
    $pdf->Cell(0, 10, 'FOOD ALLERGIES: ' . $employeeData['FoodAllergies'], 0, 1); // Added
